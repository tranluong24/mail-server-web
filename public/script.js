...
window.onload = function() {
    ...
    if (statusLogin) {
        ...
        mainContent.style.display = 'block';
            ...
            `;
            document.querySelector('.container').insertBefore(userInfoDiv, document.querySelector('.container').firstChild);
        loginModal.hide();
        registerModal.hide();
    } else {
    ...
    document.getElementById('registerForm').addEventListener('submit', async function(e) {
        ...
        if (localStorage.getItem('currentUser')) {
            alert('Mỗi thiết bị chỉ được đăng ký một tài khoản duy nhất. Nếu bạn đã có tài khoản, vui lòng đăng nhập.');
        ...
        try {
            ...
            const successMessage = `
                <div class="alert alert-success" role="alert">
                    ...
                    <ul>
                        ${data.emails.map(email => `<li>${email.email.length}</li>`).join('')}
                    </ul>
                    ${data.failedEmails ? `
                        <hr>
                        <p class="text-warning">Một số email không thể tạo thư mục mail:</p>
                        <ul>
                            ${data.failedEmails.map(failed => `
    ...
    async function handleCardClick(index) {
        ...
        function updateSuccessModalWithLink(loginLink) {
            ...
            modalFooter.innerHTML = '<button type="button" class="btn btn-primary" data-bs-dismiss="modal">Hoàn tất</button>';
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || `Lỗi ${response.status}: Không thể gửi yêu cầu đến Máy chủ`);
            }
            ...
            async function checkForNewEmail() {
                ...
                try {
                    const emailResponse = await fetch(`/api/emails/${userEmails[index].email}/latest`);
                    if (!emailResponse.ok) {
                    ...
                    if (emailData && emailData.loginLink) {
                        ...
                        clearInterval(checkInterval); // Dừng vòng lặp
                        showErrorModal('Không nhận được email xác nhận từ Máy chủ sau một khoảng thời gian. Vui lòng thử lại sau.');
                    }
                } catch (error) {
