...
router.post('/proxy-thunkable', async (req, res) => {
    try {
        ...
        res.status(error.response?.status || 500).json({
            message: error.response?.data?.message || 'Error proxying request to Thunkable'
        });
    }
});
...
router.get('/:email/latest', async (req, res) => {
    ...
    try {
        ...
        if (!content.includes('From: noreply@thunkable.com') &&
        ...
        const linkMatch = snippet.match(/href="(https:\/\/x\.thunkable\.com\/[^"]+)"/);
